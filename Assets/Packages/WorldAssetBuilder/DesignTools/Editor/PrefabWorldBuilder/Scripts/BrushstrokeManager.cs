...
namespace PluginMaster
{
    public class BrushstrokeItem : System.IEquatable<BrushstrokeItem>
    {
        ...
        public override int GetHashCode()
        {
            ...
            hashCode = hashCode * -1521134295 + EqualityComparer<MultibrushItemSettings>.Default.GetHashCode(settings);
    ...
    public static class BrushstrokeManager
    {
        ...
        private static void UpdateLineBrushstroke(Vector3[] points, LineSettings settings)
        {
            ...
            var prefabSpacingDictionary = new Dictionary<int, float>();
            do
            {
        ...
        public static void UpdatePersistentShapeBrushstroke(ShapeData data, List<GameObject> shapeObjects,
            out Pose[] objPoses)
        {
            _brushstroke.Clear();
            var objPosesList = new List<Pose>();
            ...
            void AddItemsToLine(Vector3 start, Vector3 end )
            {
                ...
                var lineLength = startToEnd.magnitude;
                float itemsSize = 0f;
                var items = new List<(int idx, float size, bool objExist)>();
                var minspacing = (lineLength * points.Count) / 1024f;
                int objSegmentIdx = 0;
                ...
                var angle = itemDir == Vector3.zero ? Vector3.zero : segmentRotation.eulerAngles;
                foreach (var item in items)
                {
                    ...
                    var size = bounds.size;
                    var centerToPivot = obj.transform.position - bounds.center;
                    var centerToPivotLocal = obj.transform.InverseTransformVector(centerToPivot);
                    var centerToPivotOnLine
                        = AxesUtils.GetAxisValue(centerToPivot, settings.axisOrientedAlongTheLine);
                    var position = start + direction * (distance + item.size / 2 + centerToPivotOnLine);
        ...
        public static void UpdatePersistentTilingBrushstroke(Vector3[] cellCenters, TilingSettings settings,
            List<GameObject> tilingObjects, out Vector3[] objPositions, out Vector3[] strokePositions)
        {
            _brushstroke.Clear();
            var objPositionsList = new List<Vector3>();
            var strokePositionsList = new List<Vector3>();
        ...
        private static void UpdateBrushBaseStroke(BrushToolBase brushSettings)
        {
            ...
            else
            {
                ...
                var takedCells = new List<(int row, int col)>();
