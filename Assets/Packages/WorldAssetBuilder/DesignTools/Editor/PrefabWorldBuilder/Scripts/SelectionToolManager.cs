        ...
        private static void SelectionInput(List<Vector3> points, bool in2DMode)
        {
                ...
                && _selectedBoxPointIdx >= 0)
            {
               ...
               && Event.current.keyCode == KeyCode.LeftArrow) RotateSelection90Deg(Vector3.down, points);
                ...
                && Event.current.keyCode == KeyCode.UpArrow) RotateSelection90Deg(Vector3.forward, points);
            else if (Event.current.type == EventType.KeyDown && Event.current.control && Event.current.shift
                && Event.current.keyCode == KeyCode.DownArrow) RotateSelection90Deg(Vector3.back, points);
            else if (Event.current.type == EventType.KeyDown && Event.current.control && Event.current.keyCode == KeyCode.X)
            {
