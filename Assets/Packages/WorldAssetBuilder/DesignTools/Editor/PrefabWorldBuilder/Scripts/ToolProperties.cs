    ...
    public class ToolProperties : EditorWindow
    {
        ...
        private void PaintToolSettingsGUI(IPaintToolSettings paintSettings)
        {
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                using (var check = new EditorGUI.ChangeCheckScope())
                {
                    var autoCreateParent = EditorGUILayout.ToggleLeft("Create parent", paintSettings.autoCreateParent);
                    if (check.changed)
                    {
                ...
                using (var check = new EditorGUI.ChangeCheckScope())
                {
                    var createSubparent = EditorGUILayout.ToggleLeft("Create sub-parents per palette",
                ...
                using (var check = new EditorGUI.ChangeCheckScope())
                {
                    var createSubparent = EditorGUILayout.ToggleLeft("Create sub-parents per tool",
                ...
                using (var check = new EditorGUI.ChangeCheckScope())
                {
                    var createSubparent = EditorGUILayout.ToggleLeft("Create sub-parents per brush",
                ...
                using (var check = new EditorGUI.ChangeCheckScope())
                {
                    var createSubparent = EditorGUILayout.ToggleLeft("Create sub-parents per prefab",
                ...
                if (settings.brushShape != BrushToolBase.BrushShape.POINT)
                {
                    using (new GUILayout.VerticalScope(EditorStyles.helpBox))
                    {
                        ...
                        using (var check = new EditorGUI.ChangeCheckScope())
                        {
                            ...
                            var randomness = EditorGUILayout.Slider("Randomness:", settings.randomness, 0f, 1f);
                            if (check.changed)
                            {
        ...
        private void EditModeToggle()
        {
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                using (var check = new EditorGUI.ChangeCheckScope())
                {
                    var editMode = EditorGUILayout.ToggleLeft("Edit Mode", ToolManager.editMode);
                    if (check.changed)
                    {
        ...
        private void BrushGroup()
        {
            ...
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                if (BrushManager.settings.showPreview)
                    EditorGUILayout.HelpBox("The brushstroke preview can cause slowdown issues.", MessageType.Info);
            }
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
        ...
        private void OnBrushTagFilterChanged(List<string> prevFilter, List<string> newFilter, string key)
        {
        ...
        private void LineBaseGUI<SETTINGS>(SETTINGS lineSettings) where SETTINGS : LineSettings, new()
        {
            void OnValueChanged()
            {
                PWBIO.UpdateStroke();
            ...
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                if (lineSettings.mode != PaintOnSurfaceToolSettingsBase.PaintMode.ON_SHAPE)
                {
                    using (var check = new EditorGUI.ChangeCheckScope())
                    {
                        var parallelToTheSurface = EditorGUILayout.ToggleLeft("Place objects parallel to the surface",
                            lineSettings.parallelToTheSurface);
                        if (check.changed)
                        {
                            lineSettings.parallelToTheSurface = parallelToTheSurface;
                            OnValueChanged();
                        }
                    }
                }
                ...
                else _lineProjDirIdx = Array.IndexOf(_dir, lineSettings.projectionDirection);
            ...
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                using (var check = new EditorGUI.ChangeCheckScope())
                {
                    ...
                    = EditorGUILayout.ToggleLeft("Orient Along the Line", lineSettings.objectsOrientedAlongTheLine);
                    if (check.changed)
                    {
            ...
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                if (lineSettings.spacingType == LineSettings.SpacingType.CONSTANT)
                {
                    using (var check = new EditorGUI.ChangeCheckScope())
                    {
                        var spacing = EditorGUILayout.FloatField("Value:", lineSettings.spacing);
                        if (check.changed)
                        {
                ...
                using (var check = new EditorGUI.ChangeCheckScope())
                {
                    var gapSize = EditorGUILayout.FloatField("Gap Size:", lineSettings.gapSize);
                    if (check.changed)
                    {
        ...
        private void SelectionGroup()
        {
            ...
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                {
                    EditorGUIUtility.labelWidth = 90;
                    var paletteFilter = EditorGUILayout.ToggleLeft("Prefabs from selected palette only",
                        SelectionToolManager.settings.paletteFilter);
                    var brushFilter = EditorGUILayout.ToggleLeft("Prefabs from selected brush only",
                        SelectionToolManager.settings.brushFilter);
                    var layerMask = EditorGUILayout.MaskField("Layers:",
        ...
        private void OnSelectionTagFilterChanged(List<string> prevFilter, List<string> newFilter, string key)
        {
