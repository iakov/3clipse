            ...
            if (Event.current.button == 0)
            {
                ...
                if (Event.current.type == EventType.KeyDown)
                {
                    if (Event.current.keyCode == KeyCode.PageUp
                        ...
                        || (Event.current.control && Event.current.shift && Event.current.keyCode == KeyCode.L))
                        _pinOffset = prevBoundLayer;
                    else if (Event.current.keyCode == KeyCode.End
                        || (Event.current.control && Event.current.shift && Event.current.keyCode == KeyCode.I))
                        ...
                        || (Event.current.control && Event.current.shift && Event.current.keyCode == KeyCode.P))
                    ...
                    //add rotation around Y
                        ...
                        && Event.current.control && Event.current.shift) _pinAngle.y -= PinManager.rotationSnapValue;
                    else if (Event.current.keyCode == KeyCode.RightArrow
                        && Event.current.control && Event.current.shift) _pinAngle.y += PinManager.rotationSnapValue;
                    //add rotation around X
                    else if (Event.current.keyCode == KeyCode.Comma && !Event.current.capsLock
                        && Event.current.control && !Event.current.shift) _pinAngle.x = (_pinAngle.x + 90) % 360;
                    else if (Event.current.keyCode == KeyCode.Period && !Event.current.capsLock
                        && Event.current.control && !Event.current.shift) _pinAngle.x = (_pinAngle.x - 90) % 360;
                    else if (Event.current.keyCode == KeyCode.Comma && !Event.current.capsLock
                        && Event.current.control && Event.current.shift) _pinAngle.x -= PinManager.rotationSnapValue;
                    else if (Event.current.keyCode == KeyCode.Period && !Event.current.capsLock
                        && Event.current.control && Event.current.shift) _pinAngle.x += PinManager.rotationSnapValue;
                    //add rotation around Z
                    else if (Event.current.keyCode == KeyCode.Comma && Event.current.capsLock
                        && Event.current.control && !Event.current.shift) _pinAngle.z = (_pinAngle.z + 90) % 360;
                    else if (Event.current.keyCode == KeyCode.Period && Event.current.capsLock
                        && Event.current.control && !Event.current.shift) _pinAngle.z = (_pinAngle.z - 90) % 360;
                    else if (Event.current.keyCode == KeyCode.Period && Event.current.capsLock
                        && Event.current.control && Event.current.shift) _pinAngle.z -= PinManager.rotationSnapValue;
                    else if (Event.current.keyCode == KeyCode.Comma && Event.current.capsLock
                        && Event.current.control && Event.current.shift) _pinAngle.z += PinManager.rotationSnapValue;
                    //reset rotation
                    else if ((Event.current.keyCode == KeyCode.Home && Event.current.control
                        && !Event.current.shift && !Event.current.alt)
                        || (Event.current.control && Event.current.shift && Event.current.keyCode == KeyCode.M))
                        _pinAngle = Vector3.zero;
                        ...
                        && Event.current.control && Event.current.shift) _pinDistanceFromSurface += 0.1f;
                    else if (Event.current.keyCode == KeyCode.H && !Event.current.alt
                        && Event.current.control && Event.current.shift) _pinDistanceFromSurface = 0;
                    else if ((Event.current.keyCode == KeyCode.Home && Event.current.control && Event.current.shift)
                        || (Event.current.control && Event.current.shift && Event.current.keyCode == KeyCode.K))
                        UpdatePinScale(1f);
        ...
        private static void DrawPinHandles(Matrix4x4 rootToWorld)
        {
            ...
            foreach (var handlePoint in handlePoints)
            {
                ...
                Handles.DotHandleCap(795, handlePoint, Quaternion.identity,
                    HandleUtility.GetHandleSize(pos) * 0.0325f * PWBCore.staticData.controPointSize, EventType.Repaint);
                Handles.color = Handles.preselectionColor;
                Handles.DotHandleCap(795, handlePoint, Quaternion.identity,
                    HandleUtility.GetHandleSize(pos) * 0.02f * PWBCore.staticData.controPointSize, EventType.Repaint);
            }

            Handles.color = new Color(0f, 0f, 0f, 0.7f);
            Handles.DotHandleCap(418, _pinHit.point, Quaternion.identity,
                HandleUtility.GetHandleSize(_pinHit.point) * 0.0425f * PWBCore.staticData.controPointSize, EventType.Repaint);
            Handles.color = Handles.selectedColor;
            Handles.DotHandleCap(418, _pinHit.point, Quaternion.identity,
                HandleUtility.GetHandleSize(_pinHit.point) * 0.03f * PWBCore.staticData.controPointSize, EventType.Repaint);
        }
    }
    #endregion
}
